...
contract AgriYield is ReentrancyGuard {
    ...
    function _refundInvestors(uint256 farmId) internal {
        Farm storage f = farms[farmId];
        require(
            f.status == Status.Active || f.status == Status.Funded,
            "Not refundable"
        );
    ...
    function delistFarm(uint256 farmId) external onlyAdmin {
        ...
        // If campaign failed (not funded by deadline) or funded but delisting, refund investors
        if (
            (f.status == Status.Active && block.timestamp > f.deadline) ||
            f.status == Status.Funded
        ) {
